...
export default function MatchOnDate({ listIndex, matchesObject, month, season }) {
  ...
  return (
    <div>
      ...
      <ul key={listIndex} style={{listStyleType: 'none'}}>
        {matchesOnDate.map((match, index) => <MatchTile key={index} match={match} month={month} season={season}/>)} 
      </ul>
    </div>
  )
}
